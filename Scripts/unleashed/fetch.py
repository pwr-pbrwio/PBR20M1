...
def fetch(project_issue_code, jira_project_name, outputPath):
    """ Fetch issues that match given jql query """
    ...
    while start_at < total:
        with url.urlopen(request.format(jql, start_at, max_results)) as conn:
            with io.open(outputPath + '/res' + str(start_at) + '.json', 'w', encoding="utf-8") as f:
...
if __name__ == '__main__':
	...
	fetch(project_issue_code, jira_project_name, 'issues')
