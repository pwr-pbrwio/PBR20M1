...
if __name__ == "__main__":
  ...
  try:
    ...
    issue_list = find_bug_fixes('.temp/fetch_issues', '.temp/gitlog.json', r'{tag}-{{nbr}}\D|#{{nbr}}\D'.format(tag=tag.upper()))

    with open('.temp/issue_list.json', 'w', encoding="utf8") as f:
      f.write(json.dumps(issue_list))
    
    print('Run unleashed szz...')
    os.system(commandFormat.format(file=os.path.dirname(os.path.realpath(__file__)), repoPath=repoPath))
