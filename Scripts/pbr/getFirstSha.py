...
def getCreationDate(owner, repo):
  try:
    r = requests.get(url = githubRepoUrl.format(owner=owner, repo=repo), headers = headers)
...
def getFirstSha(owner, repo):
  ...
  r = requests.get(url = githubCommitsUrl.format(owner=owner, repo=repo), headers = headers)
  data = r.json()
